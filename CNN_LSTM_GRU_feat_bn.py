...
from keras.layers import MaxPooling1D, GlobalMaxPool1D, GlobalAvgPool1D, Dense, Input, Embedding, Dropout, CuDNNGRU, CuDNNLSTM, Conv1D, Bidirectional, concatenate, BatchNormalization
...
with open('test_new.msgpack', 'rb') as data_file:
    test = msgpack.unpack(data_file)
subm = pd.read_csv('sample_submission_new.csv')
...
weights_path = "weights_validation/cnn_lstm_gru_feat_bn.h5"
best_weights_path = "weights_best/cnn_lstm_gru_feat_bn/"
...
def build_model(features, embed_size=300, dropout=0.2, kernel_initializer='he_uniform', clipvalue=1.0, clipnorm=1.0):
...
subm_path = "submissions_validated/cnn_lstm_gru_feat_bn_{}.csv".format(int(mean_score * 10**7))
